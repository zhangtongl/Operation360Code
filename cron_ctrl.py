#!/usr/bin/env python 
# 这个需求类似于windows下的任务管理器。
#需求分析，利用一个frame 窗体 把linux用户下的crontab 定时任务 遍历到窗体中以及能检测出定时任务的状态，给这些定时任务 三个功能，（单、批量）开启/关闭：刚刚接触pythoh 实在心有余而力不足
# 下列只是python实现的解析crontab配置文件代码
